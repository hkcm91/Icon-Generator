...
/**
 * A live wallpaper that <em>is</em> a web page, rather than a reimplementation
 * of one.
 *
 * <p>The physics, the optics and the look all live in {@code ../../wallpaper}
 * and were hard-won; porting them here would mean maintaining two of
 * everything and having them disagree. So this hosts the page, and its whole
 * job is the three things a WallpaperService can do that a browser tab cannot:
 * put a view hierarchy into the wallpaper surface, feed it the sensors
 * directly, and stop it dead when the wallpaper is not on screen.
 *
 * <p>None of that is specific to any one page, which is why this class is
 * abstract and holds no opinion about which one it is showing. A subclass
 * names an asset and a colour to paint until that asset has loaded; the
 * surface, the sensors, the clock and the lifecycle are the same work whatever
 * is drawn. Every page exposes the same {@code window.__wallpaper} table, so
 * this never has to know which one it has.
 */
public abstract class WebWallpaperService extends WallpaperService {

    /** The page to host, as a {@code file:///android_asset/...} URL. */
    protected abstract String page();

    /** Painted until the page draws its first frame, so there is no flash. */
    protected abstract int loadingColour();

    /** Log tag, so two wallpapers in one APK stay separable in logcat. */
    protected abstract String tag();

    /* The page's host interface. {@code __shaker} is the name the liquid
     * shaker exposed before there was a second wallpaper to host; it is still
     * an alias on both pages, and looking for it second means a page built
     * before the rename is still driven correctly. */
    private static final String BRIDGE = "(window.__wallpaper||window.__shaker)";
    ...
    private class WebEngine extends Engine implements SensorEventListener {
        ...
        public void onCreate(SurfaceHolder holder) {
            ...
            web = new WebView(WebWallpaperService.this);
            /* The page's own ground, so there is no flash before the first
             * frame lands: white before a blue wallpaper is as wrong as black
             * before a white one. */
            web.setBackgroundColor(loadingColour());
            ...
            web.setWebViewClient(new WebViewClient() {
                ...
                public void onPageFinished(WebView view, String url) {
                    pageReady = true;
                    if (debuggable) js(BRIDGE + ".diag(true)");
                    if (hostDrivesClock) js(BRIDGE + ".drive()");
                    else if (isVisible()) js(BRIDGE + ".resume()");
                }
            });
            root = new FrameLayout(WebWallpaperService.this);
        ...
        private void startHost(SurfaceHolder holder, int width, int height) {
            ...
            try {
                vd = dm.createVirtualDisplay(tag(), width, height, density,
                        holder.getSurface(),
                        DisplayManager.VIRTUAL_DISPLAY_FLAG_PRESENTATION
                                | DisplayManager.VIRTUAL_DISPLAY_FLAG_OWN_CONTENT_ONLY);
                if (vd == null) throw new IllegalStateException("no virtual display");

                Presentation p = new Presentation(WebWallpaperService.this, vd.getDisplay());
        ...
        public void onVisibilityChanged(boolean visible) {
            ...
            if (visible) {
                ...
                if (pageReady && !hostDrivesClock) js(BRIDGE + ".resume()");
                ...
                stopSensors();
                if (pageReady && !hostDrivesClock) js(BRIDGE + ".pause()");
        ...
        public void onOffsetsChanged(float xOffset, float yOffset, float xStep, float yStep,
            ...
            if (pageReady) js(BRIDGE + ".offset(" + xOffset + ")");
        }
